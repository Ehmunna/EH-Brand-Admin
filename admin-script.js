// ===========================================
// সম্পূর্ণ ওয়ার্কিং অ্যাডমিন স্ক্রিপ্ট - মোবাইল ফ্রেন্ডলি
// ===========================================
...
function switchSection(sectionId) {
    ...
    const linkText = activeLink?.querySelector('span')?.textContent || 
                     activeLink?.querySelector('a')?.textContent?.trim() || 
                     sectionId;
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <form id="loginForm">
                ...
                <small style="display:block; margin-top:10px; color:#666;">প্রথমে Firebase Console এ ইউজার তৈরি করুন</small>
            </form>
        </div>
    `;
...
async function updateDashboardCounters() {
    try {
        ...
        const todayQuery = query(collection(db, 'orders'), where('orderTime', '>=', today));
        ...
        if (recentDiv) {
            if (recentSnap.empty) {
                ...
                recentSnap.forEach(doc => {
                    ...
                    recentDiv.innerHTML += `<div class="stats-item"><div><div class="title">${order.name || 'N/A'}</div><div class="subtitle">${time.toLocaleString()}</div></div><span class="badge-pending" style="padding:2px 8px;border-radius:20px;font-size:12px;">${order.status || 'Pending'}</span></div>`;
                });
            }
        }
        
        // পণ্যের সংখ্যা দেখানো
        const productDiv = document.getElementById('lowStockProducts');
        if (productDiv) {
            productDiv.innerHTML = `<div class="stats-item"><div><div class="title">মোট পণ্য</div><div class="subtitle">${productsSnap.size} টি পণ্য</div></div><i class="fas fa-box"></i></div>`;
        }
    } catch (error) {
...
async function loadProductsTable() {
    ...
    tbody.innerHTML = '<tr><td colspan="6" class="empty-row"><div class="loader"><div class="spinner"></div><p>লোড হচ্ছে...</p></div></td></tr>';
    
    try {
        const snapshot = await getDocs(collection(db, 'products'));
        if (snapshot.empty) {
            tbody.innerHTML = '<tr><td colspan="6" class="empty-row"><p class="empty-message">কোনো পণ্য নেই। প্রথম পণ্য যোগ করুন!</p></td></tr>';
            return;
        }
        ...
        snapshot.forEach(doc => {
            ...
            row.innerHTML = `
                <td><img src="${imgUrl}" class="product-image" onerror="this.src='https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=100&h=100&fit=crop'"></td>
                <td>${product.name || 'N/A'}</td>
                <td><code>${product.productCode || 'N/A'}</code></td>
                <td>$${parseFloat(product.price || 0).toFixed(2)}</td>
                <td>${product.discount || 0}%</td>
                <td class="actions">
                    <button class="btn-primary btn-sm edit-product" data-id="${doc.id}"><i class="fas fa-edit"></i></button>
                    <button class="btn-danger btn-sm delete-product" data-id="${doc.id}"><i class="fas fa-trash"></i></button>
                </td>
            `;
        ...
        document.querySelectorAll('.edit-product').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                if (docSnap.exists()) openProductModal({ id, ...docSnap.data() });
            });
        });
        ...
        console.error(error);
        tbody.innerHTML = `<tr><td colspan="6" class="empty-row"><p class="error-message">লোড করা যায়নি: ${error.message}</p></td></tr>`;
    }
}
...
productForm?.addEventListener('submit', async (e) => {
    ...
    if (!name || !price || !productCode) {
        showMessage('নাম, দাম এবং কোড অবশ্যই দিতে হবে!', 'error');
        return;
    }
    
    const productData = {
        name, price, discount, productCode,
